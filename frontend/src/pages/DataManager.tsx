...
function formatJobRange(job: DataSyncJobSummary): string {
  ...
  return `回溯 ${job.historyDays || 365} 天`;
}
...
function formatJobTimeframes(job: DataSyncJobSummary): string {
  const labels = (job.timeframes || []).map((timeframe) => TIMEFRAME_LABELS[timeframe] || timeframe);
...
export default function DataManager() {
  ...
  const [addSymbolHistoryDays, setAddSymbolHistoryDays] = useState(365);
  ...
  const [syncDialogStartDate, setSyncDialogStartDate] = useState(() => dateDaysAgo(180));
  ...
  const [scheduleHistoryDays, setScheduleHistoryDays] = useState('7');
  ...
  const openSyncDialog = (mode: SyncDialogMode) => {
    const days = mode === 'daily' ? 7 : mode === 'full' ? 365 : 180;
  ...
  const submitSyncDialog = async () => {
    ...
    const historyDays = mode === 'daily' ? 7 : 365;
  ...
  const openScheduleDialog = () => {
    ...
    setScheduleHistoryDays(String(schedule?.historyDays || 7));
    setScheduleSymbols(schedule?.symbols || []);
    setScheduleTimeframes(schedule?.timeframes?.length ? schedule.timeframes : [...allTimeframes]);
  ...
  const submitScheduleDialog = async () => {
    ...
    const historyDays = Math.max(1, Math.min(365, Number(scheduleHistoryDays) || 7));
    try {
      ...
      const res = await dataSyncApi.updateSchedule({
        ...
        timeframes: scheduleTimeframes.length > 0 ? scheduleTimeframes : allTimeframes,
      });
  ...
  const handleSyncOne = async (symbol: string, timeframe: string, startDate?: string, endDate?: string) => {
    try {
      ...
      setTargetFeedback(symbol, timeframe, `正在启动 ${TIMEFRAME_LABELS[timeframe] || timeframe}${dateHint} 同步任务...`, 'info');
  ...
  const openAddSymbolDialog = async () => {
    ...
    setAddSymbolHistoryDays(365);
    setShowAddSymbolDialog(true);
    setLoadingAvailableSymbols(true);
    try {
      const [spotRes, swapRes] = await Promise.all([
        marketApi.getSymbols(selectedExchange, 'USDT', 'spot'),
        marketApi.getSymbols(selectedExchange, 'USDT', 'swap'),
      ]);
      const normalized = Array.from(new Set(
        [...(spotRes.symbols || []), ...(swapRes.symbols || [])]
          .map((symbol) => normalizeUsdtCandidate(symbol))
          .filter((symbol): symbol is string => Boolean(symbol))
      )).sort((a, b) => {
        const aSwap = isUsdtSwapSymbol(a);
        const bSwap = isUsdtSwapSymbol(b);
  ...
  const handleAddSymbol = async () => {
    ...
    try {
      ...
      setConfig((prev) => prev
        ...
        : {
            defaultSymbols,
            defaultTimeframes: allTimeframes,
  ...
  const selectAddSymbolGroup = (group: AddSymbolGroup) => {
    const choices = [group.spotSymbol, group.swapSymbol]
  ...
  const scheduleSummary = schedulePulseOn
    ? `${scheduleConfig?.intervalMinutes || 240}分钟 · ${(scheduleConfig?.timeframes || []).map((tf) => TIMEFRAME_LABELS[tf] || tf).join('/') || '全部周期'}`
    : '未启用';
  ...
  const handleSyncMissingTimeframes = async (symbol: string) => {
    const missingTimeframes = allTimeframes.filter((tf: string) => {
      const stat = statMap.get(`${symbol}_${tf}`);
  ...
  const renderOperationRow = (job: DataSyncJobSummary) => {
    ...
    const timeframeTitle = formatFullList(job.timeframes || [], (timeframe) => TIMEFRAME_LABELS[timeframe] || timeframe);
          ...
          <div className="dataHelpTooltip group/data-help relative">
            ...
            <div className="pointer-events-none absolute right-0 top-11 z-30 w-[560px] max-w-[calc(100vw-3rem)] rounded-xl border border-blue-500/25 bg-[#111827] p-4 text-xs leading-relaxed text-gray-400 shadow-2xl shadow-black/40 opacity-0 translate-y-1 transition-all duration-150 group-hover/data-help:opacity-100 group-hover/data-help:translate-y-0 group-focus-within/data-help:opacity-100 group-focus-within/data-help:translate-y-0">
              <div className="space-y-1.5">
                <p><strong className="text-gray-200">全量同步</strong> — 拉取每个交易对最近 <strong className="text-gray-200">1 年</strong> 的 1m/5m/15m/1h/4h/1d 数据，首次需要较长时间</p>
                <p><strong className="text-gray-200">自定义同步</strong> — 选择日期范围、币种和周期，<strong className="text-gray-200">精确指定</strong>要拉取的历史数据</p>
                <p><strong className="text-gray-200">增量更新</strong> — 从上次同步位置继续拉取新数据，同时回溯 <strong className="text-gray-200">7 天</strong> 以防断档。适合日常使用</p>
                ...
                <p><strong className="text-gray-200">分表存储</strong> — 每个周期独立表 (kline_1m, kline_5m, kline_15m, kline_1h, kline_4h, kline_1d)，查询性能更优</p>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[60] bg-black/70 backdrop-blur-[2px] flex items-center justify-center p-4">
          <div className="w-full max-w-5xl max-h-[calc(100vh-3rem)] overflow-y-auto rounded-2xl border border-emerald-500/35 bg-[#0f1624] shadow-2xl shadow-black/50">
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              <div className="flex flex-wrap items-center gap-5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setScheduleHistoryDays(e.target.value)}
                    className="h-9 w-28 bg-gray-800 border border-crypto-border rounded-lg px-3 text-sm text-white focus:outline-none focus:border-emerald-500 transition"
                  />
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button
                    onClick={() => setScheduleSymbols(scheduleSymbols.length === configuredSymbols.length ? [] : [...configuredSymbols])}
                    className="text-[10px] text-emerald-400 hover:text-emerald-300 transition"
                  >
                    {scheduleSymbols.length === configuredSymbols.length ? '取消全选' : '全选'}
                  </button>
                  {scheduleSymbols.length === 0 && (
                    <span className="text-[10px] text-gray-600">（不选 = 全部）</span>
                  )}
                </div>
                <div className="flex max-h-32 flex-wrap gap-1.5 overflow-y-auto rounded-lg border border-crypto-border bg-gray-900/35 p-2">
                  {configuredSymbols.map((sym) => {
                    const coin = getCoinBase(sym);
                    const selected = scheduleSymbols.includes(sym);
                    const color = COIN_COLORS[coin] || '#8B8B8B';
                    return (
                      <button
                        key={sym}
                        onClick={() =>
                          setScheduleSymbols(selected
                            ? scheduleSymbols.filter(s => s !== sym)
                            : [...scheduleSymbols, sym]
                          )}
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button
                    onClick={() => setScheduleTimeframes(scheduleTimeframes.length === allTimeframes.length ? [] : [...allTimeframes])}
                    className="text-[10px] text-emerald-400 hover:text-emerald-300 transition"
                  >
                    {scheduleTimeframes.length === allTimeframes.length ? '取消全选' : '全选'}
                  </button>
                  {scheduleTimeframes.length === 0 && (
                    <span className="text-[10px] text-gray-600">（不选 = 全部周期）</span>
                  )}
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {allTimeframes.map((tf: string) => {
                    const selected = scheduleTimeframes.includes(tf);
                    return (
                      <button
                        key={tf}
                        onClick={() =>
                          setScheduleTimeframes(selected
                            ? scheduleTimeframes.filter(t => t !== tf)
                            : [...scheduleTimeframes, tf]
                          )}
            ...
            <div className="px-6 py-4 border-t border-crypto-border flex items-center justify-between">
              <div className="text-xs text-gray-500">
                每 {scheduleIntervalMinutes || 240} 分钟 · 回溯 {scheduleHistoryDays || 7} 天
                {scheduleSymbols.length > 0 ? ` · ${scheduleSymbols.length} 个币种` : ' · 全部币种'}
                {scheduleTimeframes.length > 0 ? ` · ${scheduleTimeframes.map((tf) => TIMEFRAME_LABELS[tf] || tf).join('/')}` : ' · 全部周期'}
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button onClick={() => setSyncDialogTimeframes(syncDialogTimeframes.length === allTimeframes.length ? [] : [...allTimeframes])}
                    className="text-[10px] text-purple-400 hover:text-purple-300 transition">
                    {syncDialogTimeframes.length === allTimeframes.length ? '取消全选' : '全选'}
                  </button>
      ...
      >
        <div className="space-y-3">
          <div>
            ...
            <div className="mt-2 text-[11px] text-gray-500">现货 + 合约可同时添加；已存在的市场会自动跳过。</div>
          </div>
                  ...
                  {addSymbolGroups.slice(0, 120).map((group) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <span className="block truncate font-semibold">{group.base}/USDT</span>
                        ...
                        <div className="grid grid-cols-2 gap-1.5 px-2 pb-2">
                          {([
                            { symbol: group.spotSymbol, label: '现货' },
          ...
          <div className="max-h-72 overflow-y-auto rounded-xl border border-crypto-border bg-crypto-bg/50 p-1.5">
            {removeSymbolCandidates.length === 0 ? (
              ...
              return (
                ...
                >
                  <span>
                    <span className="block text-sm font-semibold text-white">{formatOkxInstrumentId(symbol)}</span>
                    <span className="block text-xs text-gray-500">{coin} · {isUsdtSwapSymbol(symbol) ? 'USDT 永续' : '现货'}</span>
                  </span>
