...
export default function Monitor() {
  ...
  const liveMonitorSummary = useMemo(() => {
    ...
    const todayRealizedPnl = todayOrders.reduce((sum, order) => sum + finiteNumber(order.pnl), 0);
    const todayFees = todayOrders.reduce((sum, order) => sum + Math.abs(finiteNumber(order.fee)), 0);
    ...
    return {
      ...
      deployedCount: livePanelStrategies.length,
  ...
  }, [liveAccounts, liveContractPositions, liveOrders, livePanelStrategies.length]);
        ...
        <section className="monitor-overview-panel rounded-xl border border-blue-500/15 bg-crypto-card p-4">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            <SentimentCard
              ...
              sub={liveMonitorAccount ? liveMonitorAccount.name : '暂无可用账户'}
            />
            <SentimentCard
              label="实盘策略"
              value={String(liveMonitorSummary.deployedCount)}
              icon={<Activity className="w-4 h-4" />}
              color={liveMonitorSummary.deployedCount > 0 ? 'green' : 'gray'}
              sub={`${liveMonitorStatusCounts.running} 运行中 / ${liveMonitorStatusCounts.paused} 暂停`}
            />
            <SentimentCard
              label="合约持仓"
