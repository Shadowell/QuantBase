...
type CapitalVersionFilter = 'all' | '100u' | '1000u' | '10000u';
...
function normalizeInstanceCapitalVersion(inst: TradingInstance): CapitalVersionFilter | 'other' {
  ...
  if (typeof capital === 'number' && Number.isFinite(capital)) {
    ...
    if (rounded === 10000) return '10000u';
  }
...
function formatInstanceTimeframePill(value: unknown): string {
  ...
  return timeframe === 'other' ? normalized.toUpperCase() : timeframe.toUpperCase();
}
...
type SortField = 'created' | 'return' | 'win_rate' | 'profit_factor';
