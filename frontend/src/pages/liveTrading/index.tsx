...
function LiveTradingWorkspace({ modeScope }: { modeScope?: TradeMode }) {
  ...
  const assetClassCounts = useMemo(
    () => ({
      all: modeInstances.length,
      spot: modeInstances.filter((i) => i.assetClass === 'spot').length,
      contract: modeInstances.filter((i) => i.assetClass === 'contract').length,
    }),
    [modeInstances],
  );
  const instances = useMemo(
    ...
    [assetClassFilter, instanceSortMode, modeInstances],
  );
  ...
  const loadStrategies = async () => {
    try {
      ...
      const raw = Array.isArray(res) ? res : res?.strategies || res?.data || [];
  ...
  const handleCardPausePaperTrading = (inst: TradingInstance) => {
    ...
    openConfirmDialog({
      ...
      content: paused
        ...
        : `确定要暂停「${inst.name}」的模拟交易吗？暂停后保留账户曲线和历史指标。`,
  ...
  const handleCardStopPaperTrading = (inst: TradingInstance) => {
    ...
    openConfirmDialog({
      title: '关闭模拟交易',
      content: `确定要关闭「${inst.name}」的模拟交易吗？历史收益、成交和账户曲线会保留。`,
      tone: 'danger',
      confirmText: '关闭交易',
      onConfirm: async () => {
        try {
  ...
  return (
    <div className={clsx('p-6 h-full flex flex-col', view === 'detail' && 'max-w-[1920px]')}>
            ...
            <label className="flex items-start gap-3 rounded-xl border border-[#263142] bg-[#0b111d] p-3 cursor-pointer">
              ...
              <span>
                ...
                <span className="mt-1 block text-xs text-gray-500 leading-relaxed">
                  勾选后会删除该策略的成交明细、账户曲线和诊断日志；下次启动按全新一轮统计。
