...
const EQUITY_METRIC_OPTIONS: Array<{
  ...
  { value: 'returnPct', label: '收益率', axisName: '%', unit: '%', color: '#22c55e', fillColor: 'rgba(34,197,94,0.16)' },
...
const MISSING_SELECTION_LOGIC = '该策略尚未补充核心选股说明。';
    ...
    <div className="rounded-xl border border-slate-600/70 bg-[#0b111d]/98 p-3 shadow-2xl shadow-black/50 ring-1 ring-white/5 backdrop-blur">
      <div className="mb-2 flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="text-[10px] text-gray-500">{POSITION_PREVIEW_TIMEFRAME} K线 · 最近 {POSITION_PREVIEW_LIMIT} 根</div>
        </div>
...
function calculateRiskFromEquityRows(
  ...
  const latest = metricRows[metricRows.length - 1]?.equity || currentEquity || initialEquity;
  let peak = initialEquity > 0 ? initialEquity : latest;
...
export default function InstanceMonitor({
  ...
  useEffect(() => {
    ...
    import('echarts')
      ...
      .catch((err) => {
        if (!cancelled) console.error('加载权益曲线图表失败:', err);
      });
    return () => {
      cancelled = true;
    };
  ...
  const displayRiskMetrics = calculateRiskFromEquityRows(
    equityRowsForMetrics,
  ...
  const riskDescriptionItems = risk
    ? [
        ...
        {
          title: '回撤与亏损监控',
          status: `最大回撤 ${displayRiskMetrics.maxDrawdownPct.toFixed(2)}%`,
          statusClass: 'border-amber-500/30 bg-amber-500/10 text-amber-200',
          description: `本区与顶部指标使用同一最大回撤口径，同时跟踪日内亏损 ${displayRiskMetrics.dailyLossPct.toFixed(2)}%。出现连续亏损或回撤扩大时，应先查看成交、持仓和诊断事件。`,
        },
        {
          title: '仓位边界',
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {sys && (
                <span className="text-xs text-gray-500">
                  {sys.strategy} · {sys.symbol} · {sys.timeframe}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="bg-crypto-card border border-crypto-border rounded-xl overflow-hidden">
          <div className="flex items-stretch border-b border-crypto-border">
            <button
              ...
              className="flex-1 min-w-0 flex items-center gap-2 px-4 py-3 text-left text-sm font-semibold text-white hover:bg-white/5 transition-colors"
            >
              {strategyDiagOpen ? (
                  ...
                  >
                    {entry.kind === 'log' ? (
                      <div>
                        ...
                        <div className="text-gray-200 mt-1 whitespace-pre-wrap break-all">
                          {entry.text}
                        </div>
                      </div>
                    ) : (
        ...
        <section className="min-h-0 overflow-hidden rounded-xl border border-crypto-border bg-crypto-card">
          ...
          >
            <span className="flex min-w-0 items-center gap-2">
              ...
              <h3 className="truncate text-sm font-semibold text-white">当前持仓</h3>
            </span>
                    ...
                    <tr key={`${row.symbol}-${idx}`} className="border-b border-crypto-border/60 text-gray-200">
                      <td className="py-2 pr-2 font-mono">
                        <div className="inline-flex items-center gap-2">
                          <button
                            type="button"
                            aria-label={`查看 ${row.symbol} ${POSITION_PREVIEW_TIMEFRAME} 行情`}
        ...
        <section className="min-h-0 overflow-hidden rounded-xl border border-crypto-border bg-crypto-card">
          ...
          >
            <span className="flex min-w-0 items-center gap-2">
              ...
              <span className="truncate text-sm font-semibold text-white">成交与事件</span>
              <ChevronDown
                className={clsx(
                    ...
                    {trades.map((t: any, i: number) => {
                      ...
                      return (
                        <tr key={t.id ?? i} className="border-b border-crypto-border/60 text-gray-200">
                          ...
                          <td className={clsx('py-2 pr-2 text-center font-semibold', sideDisplay.className)}>{sideDisplay.label}</td>
                          <td className="py-2 pr-2 font-mono">{t.symbol ?? '—'}</td>
                          <td className="py-2 pr-2 text-right tabular-nums">
      ...
      <section className="overflow-hidden rounded-xl border border-crypto-border bg-crypto-card shadow-lg shadow-black/20">
        ...
        >
          <span className="flex min-w-0 items-start gap-3">
            ...
            <span className="min-w-0">
              <span className="block truncate text-base font-semibold text-white">账户曲线</span>
              <span className="mt-1 block text-xs text-gray-500">
      ...
      {risk && (
        <div className="bg-crypto-card border border-crypto-border rounded-xl p-4 xl:min-h-[320px] flex flex-col">
          <h3 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-green-400" />
            风控状态
          </h3>
          <div aria-label="风控说明块" className="grid gap-3 flex-1 content-start lg:grid-cols-2">
