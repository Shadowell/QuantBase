...
const LIVE_ASSET_CACHE_PREFIX = 'quantbase:live-real:asset-snapshot:v1';
...
function isRunningDeployableStrategy(strategy: LiveExecutionStrategy): boolean {
  return (
    ...
    && String(strategy.status || '').toLowerCase() === 'running'
  );
}
...
const deploymentPipelineStepBase =
  'inline-flex h-9 w-full min-w-0 items-center justify-center gap-1.5 rounded-lg border px-2 text-xs font-semibold transition-colors disabled:cursor-not-allowed disabled:border-transparent disabled:bg-transparent disabled:text-gray-500';
...
function canUseAccountForLiveDeployment(account?: LiveExecutionAccount | null): boolean {
  return Boolean(account && account.exchange === 'okx' && !account.displayOnly);
}
...
export default function LiveExecutionCenter() {
  ...
  const [accountPickerStrategyId, setAccountPickerStrategyId] = useState<number | null>(null);
  const [pendingBindAccountId, setPendingBindAccountId] = useState<string>('');
  ...
  const selectedAccountBinding = useMemo(
    () => accountBindingFor(selectedStrategy, selectedAccountId),
    [selectedAccountId, selectedStrategy],
  );
  const selectedAccountBound = Boolean(selectedStrategy && boundAccountIds(selectedStrategy).includes(selectedAccountId));
  const selectedLiveSubscriptionId = selectedAccountBinding?.liveSubscriptionId || (
    selectedStrategy?.accountId === selectedAccountId ? selectedStrategy.liveSubscriptionId : undefined
  );
  ...
  const strategyFilterCounts = useMemo(() => {
    ...
    const searchableStrategies = strategies.filter((strategy) => strategyMatchesSearch(strategy, keyword));
    return {
      all: searchableStrategies.length,
      deployable: searchableStrategies.filter(isRunningDeployableStrategy).length,
      deployed: searchableStrategies.filter((strategy) => strategyMatchesFilter(strategy, 'deployed')).length,
  ...
  const loadStrategies = useCallback(async (options: { silent?: boolean } = {}) => {
    ...
    try {
      ...
      setSelectedStrategyIds((prev) => Array.from(new Set([...prev, ...addedIds])));
  ...
  const loadAssetSnapshot = useCallback(async (options: { silent?: boolean; preferCache?: boolean } = {}) => {
    ...
    const silent = Boolean(options.silent) || Boolean(cachedSnapshot);
    const showLoading = !silent && !cachedSnapshot;
    if (!silent) {
      setAccountError('');
    }
    ...
    if (selectedAccount && !canUseAccountForLiveDeployment(selectedAccount)) {
      const detail = selectedAccount.displayOnly
        ? 'Binance API Key 已展示，未配置 Secret，暂不读取余额、持仓或订单。'
        : '当前实盘资产读取仅支持 OKX；该账户暂用于展示与跨所套利研究。';
  ...
  const bindAccountToStrategy = async (strategy: LiveExecutionStrategy, accountId: string) => {
    ...
    try {
      ...
      setPendingBindAccountId('');
      setAccountPickerStrategyId(null);
  ...
  const runPreflight = async (strategy: LiveExecutionStrategy) => {
    ...
    try {
      const res = await liveExecutionApi.preflightStrategy(strategy.strategyId, {
        accountId: selectedAccountId,
        exchange: selectedExchangeAlias,
        loopInterval: 60,
  ...
  const deployStrategy = async (strategy: LiveExecutionStrategy) => {
    ...
    setError('');
    try {
      const res = await liveExecutionApi.deployStrategy(strategy.strategyId, {
        accountId: selectedAccountId,
        exchange: selectedExchangeAlias,
        loopInterval: 60,
        startImmediately: true,
  ...
  const deployReady = Boolean(
    selectedStrategy
    && selectedAccountBound
    && activePreflight?.allPassed
    && !selectedLiveSubscriptionId
  );
  const deploymentPipelineDisabled = Boolean(selectedLiveSubscriptionId);
  const selectedStrategyBusy = Boolean(selectedStrategy && actioningId === selectedStrategy.strategyId);
  const renderDeployPipeline = () => {
    ...
    const visiblePreflight = deploymentPipelineDisabled ? undefined : activePreflight;
    ...
    return (
      <div aria-label="实盘部署流水线" className="rounded-xl border border-crypto-border bg-crypto-bg p-1">
        <div className="grid grid-cols-[minmax(0,1fr)_1px_minmax(0,1fr)_1px_minmax(0,1fr)] items-center gap-1">
          <button
            type="button"
            disabled={deploymentPipelineDisabled || !selectedAccountBound || selectedStrategyBusy}
            onClick={() => void runPreflight(selectedStrategy)}
            className={clsx(deploymentPipelineStepBase, liveActionButtonWarning)}
          >
            <span className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-current bg-crypto-card text-[10px] font-bold">1</span>
            {selectedStrategyBusy ? <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin" /> : <ShieldCheck className="h-3.5 w-3.5 shrink-0" />}
            <span className="truncate">实盘预检</span>
          </button>

          <div aria-hidden="true" className="h-7 w-px bg-crypto-border/70" />

          <div className="group relative z-20 min-w-0">
            <button
              type="button"
              disabled={deploymentPipelineDisabled || !activePreflight}
              onClick={() => {
                if (!visiblePreflight) return;
              ...
              aria-describedby={visiblePreflight ? preflightResultId : undefined}
              aria-controls={visiblePreflight ? preflightResultId : undefined}
              aria-expanded={visiblePreflight ? resultPanelPinned : undefined}
              className={clsx(
                'disabled:opacity-100',
                deploymentPipelineStepBase,
                deploymentPipelineDisabled
                  ? 'border-crypto-border text-gray-500'
                  : preflightResultButtonTone(activePreflight),
              )}
            >
              <span className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-current bg-crypto-card text-[10px] font-bold">2</span>
              {activePreflight?.allPassed ? (
                <ShieldCheck className="h-3.5 w-3.5 shrink-0" />
              ) : (
                <AlertTriangle className="h-3.5 w-3.5 shrink-0" />
              )}
              <span className="truncate">预检结果</span>
            </button>
            {visiblePreflight && (
              <div
              ...
              >
                <div className="mb-2 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-sm font-semibold text-gray-100">预检结果</div>
          ...
          <button
            type="button"
            disabled={deploymentPipelineDisabled || !deployReady || selectedStrategyBusy}
            onClick={() => openDeployConfirm(selectedStrategy)}
            className={clsx(deploymentPipelineStepBase, liveActionButtonDanger)}
          >
            <span className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-current bg-crypto-card text-[10px] font-bold">3</span>
            <Rocket className="h-3.5 w-3.5 shrink-0" />
            <span className="truncate">部署实盘</span>
          </button>
        </div>
      </div>
    );
        ...
        <div className="p-3">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
            <div className="rounded-md px-1 py-1.5">
              <div className="font-mono text-sm font-semibold text-blue-300">{fundingAssetCount}</div>
              <div className="mt-0.5 text-[10px] text-gray-500">资金账户币种</div>
            </div>
            <div className="rounded-md px-1 py-1.5">
              <div className="font-mono text-sm font-semibold text-emerald-300">{tradingAssetCount}</div>
              <div className="mt-0.5 text-[10px] text-gray-500">交易账户币种</div>
            </div>
            <div className="rounded-md px-1 py-1.5">
              <div className="font-mono text-sm font-semibold text-amber-300">{contractPositions.length}</div>
              <div className="mt-0.5 text-[10px] text-gray-500">合约持仓</div>
            </div>
            <div className="rounded-md px-1 py-1.5">
              <div className="font-mono text-sm font-semibold text-gray-100">{spotPositions.length}</div>
              <div className="mt-0.5 text-[10px] text-gray-500">现货资产</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="flex h-full flex-col gap-5 p-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-xs text-gray-500">OKX API · Binance 账户展示 · 策略加入式部署</div>
        </div>
        <button
          type="button"
          onClick={() => {
      ...
      <section className="min-w-0 rounded-xl border border-crypto-border bg-crypto-card p-4">
          <div className="mb-3 flex flex-wrap items-start justify-between gap-3 border-b border-crypto-border pb-3">
            <div className="flex min-w-0 items-center gap-2">
              <KeyRound className="h-5 w-5 text-red-300" />
              <div className="min-w-0">
                <div className="truncate text-sm font-semibold text-white">实盘账户管理</div>
                <div className="text-[11px] text-gray-500">选择账户 / 新增 OKX API Key / Binance 只读展示</div>
              </div>
            </div>
            <div className="flex min-w-0 flex-1 flex-wrap items-center justify-end gap-2 sm:flex-nowrap">
              <CryptoSelect
                ...
                controlSize="xs"
                wrapperClassName="min-w-[280px] flex-1 sm:w-[360px] sm:flex-none"
              >
                {accounts.map((account) => (
                  <option key={account.accountId} value={account.accountId}>
                    {account.name} · {accountExchangeLabel(account)}
                  </option>
                ))}
              </CryptoSelect>
              {!readOnly && (
            ...
            {selectedStrategies.length === 0 ? (
              ...
              <div className="min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
                {selectedStrategies.map((strategy) => {
                  ...
                  const pickerOpen = accountPickerStrategyId === strategy.strategyId;
                  const accountOptions = accounts.filter((account) =>
                    !strategyAccountIds.includes(account.accountId) && canUseAccountForLiveDeployment(account),
                  );
                              ...
                              <div className="rounded-md border border-crypto-border bg-crypto-bg/70 p-2">
                                ...
                                {accountOptions.length === 0 ? (
                                  <div className="rounded-md border border-dashed border-crypto-border px-3 py-2 text-xs text-gray-500">
                                    当前策略已绑定全部可用 OKX 实盘账户；Binance 本阶段仅展示。
                                  </div>
                                ) : (
                                  <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center">
                                    <CryptoSelect
                                      value={pendingBindAccountId}
                                      onChange={(event) => setPendingBindAccountId(event.target.value)}
                                      controlSize="sm"
                                      wrapperClassName="min-w-0 flex-1"
                                    >
                                      <option value="">选择一个实盘账户</option>
                                    ...
                                    <button
                                      type="button"
                                      disabled={!pendingBindAccountId || accountBindingAction === `${strategy.strategyId}:${pendingBindAccountId}`}
                                      onClick={() => void bindAccountToStrategy(strategy, pendingBindAccountId)}
                                      className="inline-flex h-9 shrink-0 items-center justify-center gap-1 rounded-md border border-crypto-border bg-crypto-bg px-3 text-xs font-semibold text-gray-100 hover:border-blue-500/35 hover:text-blue-200 hover:bg-white/[0.03] disabled:cursor-not-allowed disabled:opacity-50"
                                    >
                                      <Link2 size={13} />
                                      绑定
                                    </button>
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <span
                                      className={clsx(
                                        'shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold',
                                        bound ? 'bg-green-500/15 text-green-300' : 'bg-gray-500/15 text-gray-400',
                                      )}
                                    >
                                      {bound ? '已绑定' : '未绑定'}
                                    </span>
