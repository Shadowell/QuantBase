...
const HOME_SUMMARY_SYMBOLS = Array.from(new Set([
  ...MARKET_TAB_SYMBOLS.futures,
  ...NEW_LISTING_SYMBOLS,
  ...TRADFI_SYMBOLS,
]));
...
type HomeRankingKey = 'hot' | 'new' | 'tradfi' | 'gainers' | 'losers';
...
function HomeRankingBoard({
  ...
  return (
    <div className="mt-3 grid grid-cols-1 gap-3 xl:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="overflow-hidden rounded-lg border border-crypto-border bg-crypto-card/90">
        <div className="border-b border-crypto-border/60 px-4 py-3">
          <div className="flex items-center gap-2">
      ...
      <section className="overflow-hidden rounded-lg border border-crypto-border bg-crypto-card/90">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-crypto-border/60 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="mt-0.5 text-[11px] text-gray-500">OKX ticker · {activeTab.desc}</div>
            </div>
          </div>
          <span className="rounded border border-white/10 bg-white/[0.04] px-2 py-1 text-[10px] text-gray-500">
            {items.length} / {HOME_RANKING_LIMIT} 个标的
          </span>
        </div>

        <div className="h-[560px] overflow-auto">
          <div className="min-w-[1120px]">
            <div className="sticky top-0 z-10 grid grid-cols-[minmax(160px,1fr)_116px_92px_128px_132px_132px_minmax(260px,1fr)] items-center gap-x-3 border-b border-crypto-border/50 bg-gray-950/90 px-4 py-2 text-xs text-gray-500 backdrop-blur">
              <span>标的</span>
              <span>最新价</span>
              <span>24h 涨跌</span>
              ...
              <div className="divide-y divide-crypto-border/40">
                {items.map((item, index) => (
                  <button
                    ...
                    type="button"
                    onClick={() => onSelect(item.symbol)}
                    className="grid w-full grid-cols-[minmax(160px,1fr)_116px_92px_128px_132px_132px_minmax(260px,1fr)] items-center gap-x-3 px-4 py-3 text-left transition-colors hover:bg-gray-800/45"
                  >
                    <span className="flex min-w-0 items-center gap-3">
                      ...
                      <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${COIN_COLORS[item.coin] || 'bg-gray-600'} text-xs font-bold text-white`}>
                        {item.coin.charAt(0)}
                      </span>
                      <span className="min-w-0">
                        <span className="flex min-w-0 items-center gap-2">
                          <span className="truncate text-sm font-semibold text-gray-100">{item.displayName}</span>
                          {item.isContract && (
                    ...
                    <span className="font-mono text-sm tabular-nums text-gray-100">
                    ...
                    <span className="truncate font-mono text-sm tabular-nums text-gray-400" title={`${item.volume} ${item.coin}`}>
    ...
    <section className="overflow-hidden rounded-xl border border-crypto-border bg-crypto-card/95">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-crypto-border/60 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-base font-semibold text-gray-100">OKX 市场概览 · 大盘情绪指数</h2>
        </div>
        <div className="rounded-lg border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs text-gray-400">
      ...
      <div className="grid grid-cols-1 gap-3 p-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-[280px_repeat(5,minmax(0,1fr))]">
        <div className="rounded-xl border border-white/10 bg-gray-950/45 p-4 2xl:row-span-2">
          <div className="text-xs text-gray-500">综合分</div>
          <div className={`mt-3 text-6xl font-black leading-none tabular-nums ${scoreTone}`}>
            {sentiment.score}
          </div>
        ...
        {sentiment.components.map((metric) => {
          ...
          return (
            <div key={metric.key} className={`rounded-xl border ${tone.border} ${tone.bg} p-3`}>
              ...
              <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-gray-900/70">
                <div className={`h-full rounded-full ${tone.bar}`} style={{ width: `${metric.score ?? 0}%` }} />
              </div>
...
export default function MarketUniversePanel({
  ...
  const [favorites, setFavorites] = useState<Set<string>>(() => {
    const saved = localStorage.getItem('quantbase_favorites');
  ...
  const toggleFavorite = (symbol: string) => {
    setFavorites((prev) => {
      ...
      localStorage.setItem('quantbase_favorites', JSON.stringify([...next]));
      return next;
    });
  };
  ...
  const requestedSymbols = useMemo(() => {
    if (isSummary) return HOME_SUMMARY_SYMBOLS;
  ...
  const fetchAllTickers = useCallback(async () => {
    try {
      ...
      const tickersData = await marketApi.getTickers(selectedExchange, requestedSymbols);
      setTickers(mapTickerData(tickersData as RealtimeTicker[]));
    } catch (err) {
      console.error('Failed to fetch tickers:', err);
    }
  }, [selectedExchange, requestedSymbols, mapTickerData]);
  ...
  const homeRankings = useMemo<Record<HomeRankingKey, TickerData[]>>(() => ({
      ...
      {isSummary ? (
        <>
          ...
          <HomeRankingBoard
            ...
            items={homeRankings[homeRankingKey]}
          ...
          displayedTickers.map((t) => (
            ...
            >
              ...
              <div className="flex items-center space-x-3">
                <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${COIN_COLORS[t.coin] || 'bg-gray-600'} text-xs font-bold text-white`}>
                  {t.coin.charAt(0)}
                </div>
