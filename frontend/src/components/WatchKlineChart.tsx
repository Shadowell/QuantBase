...
const APP_DEFAULT_VISIBLE_CANDLES = 42;
...
export default function WatchKlineChart({
  ...
  const option = useMemo(() => {
    ...
    const pricePadding = Math.max((chartMaxPrice - chartMinPrice) * 0.025, Math.abs(chartMaxPrice || 1) * 0.0005);
    ...
    return {
      ...
      grid: [
        { left: compact ? 34 : 56, right: compact ? 46 : 68, top: compact ? 34 : 42, height: '52%' },
        { left: compact ? 34 : 56, right: compact ? 46 : 68, top: '62%', height: '14%' },
        { left: compact ? 34 : 56, right: compact ? 46 : 68, top: '80%', height: '14%' },
      ],
  ...
  return (
    <section className={`rounded-xl border border-crypto-border bg-crypto-card/80 ${compact ? 'p-3' : 'p-4'}`}>
      <div className={`${compact ? 'mb-2 gap-2' : 'mb-3'} flex items-center justify-between`}>
        <div>
          <div className={`${compact ? 'text-xs' : 'text-sm'} font-semibold text-gray-100`}>{symbol}</div>
          <div className="text-xs text-gray-500">EMA5 / EMA10 / EMA20 · VOL · MACD · {timeframe}</div>
        </div>
