...
const liveContractPositionsPanelShell = 'watchPositionsColumn flex h-[420px] min-h-0 min-w-0 flex-col overflow-hidden rounded-xl border border-crypto-border bg-crypto-bg';
...
function orderSourceLabel(order: LiveExecutionOrder): { primary: string; secondary: string; className: string } {
  if (order.quantbaseSource === 'strategy' && order.sourceStrategyName) {
    return {
      primary: order.sourceStrategyName,
      secondary: 'QuantBase 策略信号',
      className: 'text-blue-200',
    };
  }
  return {
    primary: order.quantbaseSourceLabel || '手动/外部订单',
    secondary: '外部/手动',
...
function orderHasFailureLog(order: LiveExecutionOrder): boolean {
  ...
  return Boolean(
    order.failureLog
    || order.error
    || order.source === 'quantbase_live_execution'
    || normalized === 'failed'
    || normalized === 'rejected',
  );
}
...
export function LiveOrderDetailsPanel({
  ...
  const strategyOrderCount = visibleOrders.filter((order) => order.quantbaseSource === 'strategy' || order.sourceStrategyName).length;
