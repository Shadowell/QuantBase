...
export interface KlineChartProps {
  ...
  showEMA?: boolean;
  emaPeriods?: number[];
  /** 后端行情指标接口返回的指标序列，key 例如 EMA5 / EMA10。 */
...
export default function KlineChart({
  ...
  const option = useMemo(() => {
    ...
    return {
      ...
      grid: showVolume && hasReal
        ? [
            { left: 60, right: gridRightPx, top: 60, height: '55%' },
            { left: 60, right: gridRightPx, top: '72%', height: '18%' },
          ]
        : [{ left: 60, right: gridRightPx, top: 60, bottom: 50 }],
      xAxis: showVolume && hasReal
        ? [
            {
      ...
      dataZoom: [
        {
          type: 'inside',
          xAxisIndex: showVolume && hasReal ? [0, 1] : [0],
