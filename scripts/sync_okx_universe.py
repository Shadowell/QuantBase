#!/usr/bin/env python3
"""同步 OKX 全市场 USDT 标的历史 K 线到 QuantBase 数据模块。

特性：
- 自动发现 OKX 全市场可用的 USDT 现货和 USDT 永续合约标的
- 先写入数据模块配置，使这些标的可在 `/data` 页面被查询
- 按固定顺序同步时间粒度：1d -> 4h -> 1h -> 30m -> 15m -> 1m
- 每次只同步一个 K 线粒度，但该粒度内按 5 并行跑全市场标的
- 每个粒度完成后输出一次阶段摘要；全部完成后输出总摘要

示例：
  python3 scripts/sync_okx_universe.py
  DB_PATH=data/crypto_data.db python3 scripts/sync_okx_universe.py --start-date 2025-05-01 --concurrency 5
"""
...
FULL_SYNC_TIMEFRAME_ORDER = ["1d", "4h", "1h", "30m", "15m", "1m"]
...
def normalize_timeframe_sequence(requested: Iterable[str]) -> List[str]:
    ...
    for timeframe in requested_list:
...
def select_supported_usdt_symbols(markets: Iterable[Dict[str, Any]]) -> List[str]:
    ...
    for market in markets:
        ...
        if not symbol or not market.get("active"):
            continue
...
def sync_symbol_timeframe(
    *,
    ...
    if meta and meta.get("last_timestamp"):
        start_ms = max(start_ms, int(meta["last_timestamp"]) + interval_ms)
    ...
    while current_ms < end_ms:
        try:
            ...
            if last_ts <= current_ms and len(klines) < max_per_request:
                break
...
def ensure_data_manager_symbols(all_symbols: List[str]) -> Dict[str, int]:
    ...
    raw_custom = db.get_app_setting(custom_key, "[]")
    try:
        existing_custom = json.loads(raw_custom or "[]")
    except (TypeError, json.JSONDecodeError):
        existing_custom = []
    if not isinstance(existing_custom, list):
        existing_custom = []

    merged_custom = _dedupe([
        *[str(value) for value in existing_custom],
        *[symbol for symbol in all_symbols if symbol not in default_symbols],
    ])

    db.set_app_setting(custom_key, json.dumps(merged_custom, ensure_ascii=False))
    db.set_app_setting(removed_key, json.dumps([], ensure_ascii=False))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="同步 OKX 全市场 USDT 标的 K 线到 QuantBase 数据模块")
    parser.add_argument("--exchange", default=DEFAULT_EXCHANGE, help="交易所名称，默认 okx")
    parser.add_argument("--start-date", default=DEFAULT_START_DATE, help="同步起始日期，格式 YYYY-MM-DD")
    parser.add_argument("--end-date", default=None, help="同步结束日期，格式 YYYY-MM-DD，默认到当前时间")
    parser.add_argument(
        "--timeframes",
        default=",".join(FULL_SYNC_TIMEFRAME_ORDER),
        help="按逗号分隔的时间粒度列表，默认 1d,4h,1h,30m,15m,1m",
    )
