...
class ExchangeManager:
    ...
    def _get_live_account_exchange(self, name: str) -> Optional[BaseExchange]:
        """Lazily build a per-account OKX exchange for names like okx:<account_id>."""
        normalized = str(name or "").strip().lower()
        if ":" not in normalized:
            return None
        base, account_id = normalized.split(":", 1)
        if base != "okx" or not account_id:
            return None
    ...
    def is_supported(self, name: str) -> bool:
        ...
        return normalized in self.EXCHANGE_CLASSES or normalized.startswith("okx:")
