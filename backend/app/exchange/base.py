...
class BaseExchange(ABC):
    ...
    def fetch_tickers(self, symbols: List[str] = None) -> List[Dict]:
        ...
        if symbols:
            wants_derivatives = any(":" in s or "-SWAP" in s.upper() for s in symbols)
            if wants_derivatives:
                try:
                    swap_tickers = self.exchange.fetch_tickers(params={"instType": "SWAP"})
                    if isinstance(swap_tickers, dict):
                        all_tickers.update(swap_tickers)
                    else:
                        for ticker in swap_tickers or []:
