...
class ArbitrageDomainService:
    ...
    async def summary(self) -> Dict[str, Any]:
        ...
        universe = self._rank_common_universe(okx_snapshots, binance_snapshots)
        if not universe:
        ...
        opportunities = await self._build_opportunities(universe, okx_snapshots, binance_snapshots, spread_matrix)
        opportunities.sort(key=lambda row: row.get("net_edge_bps") or 0.0, reverse=True)
    ...
    async def _build_opportunities(
        ...
        for symbol in universe:
            ...
            if net_edge_without_depth < self.min_net_edge_bps:
                continue
        ...
        for candidate, depth_usdt in zip(candidates, depths):
            if depth_usdt is None or depth_usdt < self.min_depth_usdt:
                continue
