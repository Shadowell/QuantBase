...
def validate_live_deployable_account_id(account_id: Optional[str]) -> str:
    ...
    if _exchange_for_account(account) != "okx" or account.get("display_only"):
        raise BadRequestError("Binance 账户本阶段仅用于展示与跨所套利研究，不能用于实盘绑定、预检、部署或私有账户读取")
    return normalized
...
def validate_okx_account_permissions(
    *,
    ...
    The Trade probe cancels a randomly generated client order ID that QuantBase has
...
def validate_account_trade_permission(account_id: Optional[str]) -> Dict[str, Any]:
    """Re-run the non-executing OKX read + Trade permission probe for a saved account."""
    normalized = validate_account_id(account_id)
    account = get_account(normalized, reveal_secret=True)
    if not account:
        raise BadRequestError("实盘账户不存在，请先在顶部账户区新增 API Key")
    if _exchange_for_account(account) != "okx":
        raise BadRequestError("当前实盘交易权限测试仅支持 OKX；Binance 账户本阶段仅用于展示与跨所套利研究")
...
def create_account(
    *,
    ...
    account_id = f"okx_{_slug(clean_name)}_{secrets.token_hex(3)}"
    conn = db.get_connection()
    conn.execute(
        """
