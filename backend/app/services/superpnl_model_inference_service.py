...
class SuperPnLModelInferenceService:
    ...
    async def backfill_history_from_exchange(
        ...
        *,
        ...
        throttle = (
            ...
            else float(os.environ.get("QUANTBASE_OHLCV_MIN_INTERVAL_SEC", "0.20"))
        )
    ...
    async def get_signal(self, symbol: str, timestamp_ms: int, horizon: str = "15m") -> SuperPnLSignal | None:
        ...
        return signals.get(normalize_quantbase_symbol(symbol))
    ...
    def _load_model_package(
        ...
        symbols = universe.get("symbols_quantbase") or [
            normalize_quantbase_symbol(s) for s in universe.get("symbols_superpnl", [])
        ]
    ...
    def _resolve_cache_dir(config_value: str | None) -> str | None:
        value = config_value or os.environ.get("SUPERPNL_MODEL_CACHE_DIR") or "data/artifacts/superpnl"
        value = value.strip()
        if value in {"", "${SUPERPNL_MODEL_CACHE_DIR}"}:
            return os.environ.get("SUPERPNL_MODEL_CACHE_DIR") or "data/artifacts/superpnl"
        return value
