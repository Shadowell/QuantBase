...
`/api/v1/backtest/run_sync` 仅通过 ``get_strategy_for_id`` → BaseStrategy 运行。
...
class BacktraderBroker:
    ...
    def _data_for_symbol(self, symbol: str):
        """把 QuantBase 的 symbol 字符串映射回 Backtrader data feed。"""
    ...
    async def open_contract(
        ...
        if ref is not None:
            ...
            # 所以把 QuantBase 侧的合约元数据挂到 order ref，等 notify_order 成交时合并进明细。
...
class BTStrategyAdapter(bt.Strategy):
    ...
    def __init__(self):
        ...
        # BaseStrategy 接收的是 QuantBase 语义的 symbols 列表；Backtrader 接收的是多个 data feed。
    ...
    def next(self):
        ...
        # Backtrader 每次 next 可能同时推进多个 data feed。QuantBase 的 BaseStrategy.on_bar 是单 bar
    ...
    @staticmethod
    def _run_async(coro):
        """在同步的 Backtrader next() 中驱动用户的 async 策略方法。"""
        try:
...
class BacktestEngine:
    ...
    def run_strategy(
        ...
        QuantBase API 层使用的 BacktestReport。这里不生成 mock K 线；数据不足会抛出明确错误。
        """
        ...
        for _page in range(max_pages):
            ...
            request = Request(
                ...
                headers={
                    ...
                    "User-Agent": "QuantBase/1.0 (+https://github.com/Shadowell/QuantBase)",
                },
            )
            try:
    ...
    def _read_cached_dataframe(
        ...
        raw_df = raw_df.sort_values("timestamp").drop_duplicates(subset=["timestamp"], keep="last")
