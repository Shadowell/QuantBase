...
class StrategyService:
    ...
    async def get_strategies_page(
        ...
        *,
        ...
        旧 `/strategies` 调用仍返回完整数组；分页调用只返回当前页，避免策略库首屏
        一次性传输全部策略源码和配置。
        """
        ...
        if normalized_capital not in {"ALL", "100U", "1000U", "10000U"}:
        ...
        capital_counts = {"all": 0, "100U": 0, "1000U": 0, "10000U": 0}
        for item in count_source:
    ...
    async def get_all_running(self, *, refresh_marks: bool = False) -> List[Dict]:
        """获取所有运行中的策略"""
        return strategy_engine.get_all_running(refresh_marks=refresh_marks)
