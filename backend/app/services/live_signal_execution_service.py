...
class LiveSignalExecutionService:
    ...
    WATCHLIST_EXCLUDED_EXECUTION_STATUSES = {"failed", "rejected", "canceled", "cancelled"}
    ...
    def enrich_orders_with_attribution(
        ...
        *,
        ...
        """Attach QuantBase strategy/subscription attribution to exchange order rows."""
        self.ensure_schema()
        normalized_account = _normalize_account_id(account_id)
        ...
        for row in rows:
            ...
            attr = {
                "quantbase_source": "strategy",
                "quantbase_source_label": source_name,
        ...
        for order in order_list:
            ...
            if attr:
                ...
                enriched.append(
                    {
                        **order,
                        "quantbase_source": "external",
                        "quantbase_source_label": "手动/外部订单",
    ...
    def list_watchlist_items(self, *, account_id: str, limit: int = 100) -> List[Dict[str, Any]]:
        """Return symbols that have QuantBase strategy live execution records."""
        self.ensure_schema()
        normalized_account = _normalize_account_id(account_id)
    ...
    def list_live_order_updates(
        ...
        *,
        ...
        """Return QuantBase live execution updates for the private order bridge."""
        self.ensure_schema()
        normalized_account = _normalize_account_id(account_id)
        conn = self.db.get_connection()
        rows = conn.execute(
    ...
    def _failed_execution_order_from_row(self, row: Any) -> Dict[str, Any]:
        ...
        return {
            ...
            "signal_event_id": int(row["signal_event_id"]),
            "live_execution_id": int(row["id"]),
            "source": "quantbase_live_execution",
            "error": error,
            "failure_log": {
                "title": "OKX 拒单/实盘执行失败",
                "status": raw_status,
    ...
    async def _dispatch_contract_signal(
        ...
        *,
        ...
        broker_factory = self.contract_broker_factory or LiveContractBroker
