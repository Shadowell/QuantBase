...
This module is intentionally deterministic and LLM-free.  It gives Hermes/QuantBase a
    ...
    return {
        ...
        "agents": [
            ...
            },
            {
                ...
                "role": "只生成 QuantBase paper/simulation trade_intent，不直接触碰 OKX 实盘。",
...
class HermesAgentBridge:
    ...
    def run(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            command = self._materialize_command(prompt)
    ...
    def _materialize_command(self, prompt: str) -> List[str]:
        if "{prompt_file}" in self.command:
            with tempfile.NamedTemporaryFile("w", encoding="utf-8", suffix=".md", delete=False) as fp:
                fp.write(prompt)
                prompt_file = fp.name
            return shlex.split(self.command.replace("{prompt_file}", shlex.quote(prompt_file)))
        if "{prompt}" in self.command:
            return shlex.split(self.command.replace("{prompt}", shlex.quote(prompt)))
...
def build_hermes_research_prompt(cycle_result: Dict[str, Any]) -> str:
    ...
    return f"""你是 QuantBase 服务器本地 Hermes 策略研发 Agent。请基于下面五 Agent 本地闭环结果继续研发，但必须遵守边界：
...
4. 如要写策略，必须符合 QuantBase BaseStrategy 合约，并先经过回测和模拟盘。
...
class AiStrategyAssistantCycle:
    ...
    def _execution_plan(self, trade_intent: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            "agent": "execution_agent",
            "target": "QuantBase paper/simulation only",
            "live_trading_allowed": False,
