...
FACTOR_FAMILIES: List[FactorFamily] = [
    ...
    FactorFamily(
        ...
        evidence=(
            ...
            "QuantBase 当前研发币池已默认限定高流动性 OKX 现货。"
        ),
        quantbase_hypothesis=(
...
def build_factor_research_context(symbol_scope: str = "", timeframe: str = "", market_type: str = "spot") -> str:
    ...
    header = [
        ...
        "这些因子来自主流量化/学术研究的长期证据，但不是收益保证；每个策略仍必须在 QuantBase 回测中验证。",
    ...
    for idx, family in enumerate(FACTOR_FAMILIES, start=1):
        lines.extend([
            ...
            f"   - QuantBase 假设: {family.quantbase_hypothesis}",
