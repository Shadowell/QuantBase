...
class StrategyOptimizerService:
    ...
    async def _generate_candidate(self, source: Dict[str, Any], snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        messages = [
            {
                "role": "system",
                "content": "你是 QuantBase 量化策略优化工程师，只返回 JSON，不要输出 Markdown。",
            },
        ...
        for attempt in range(MAX_CANDIDATE_CODE_RETRIES):
            if attempt > 0 and last_error:
                messages.append({
                    "role": "user",
                    "content": (
                        "上一次优化候选代码没有通过 QuantBase 当前策略合约校验：\n"
                        f"{last_error}\n\n"
                        "请重新输出完整 JSON。必须改写为当前 BaseStrategy 合约；"
                        "禁止 quantbase.strategy、quantbase.data、Strategy/DataFeed 等旧框架导入或 API；"
        ...
        raise RuntimeError(
            f"AI 在 {MAX_CANDIDATE_CODE_RETRIES} 次尝试后仍无法生成符合 QuantBase 合约的优化候选: {last_error}"
        )
    ...
    def _build_optimizer_prompt(self, source: Dict[str, Any], snapshot: Dict[str, Any]) -> str:
        ...
        return f"""请优化下面这个正在模拟盘运行但 4 小时收益率为负的 QuantBase 策略。

要求：
1. 默认优先调整参数和风控；如果收益问题来自逻辑缺陷，可以改策略代码。
2. 必须输出一个完整的 BaseStrategy 子类，保留 async on_init / async on_bar 结构。
3. 必须使用真实 K 线和 QuantBase 当前可用数据；禁止 mock、随机信号、外部网络、文件读写、CCXT 直连。
...
7. 原策略代码可能来自旧框架，不能照抄旧导入；禁止 `from quantbase.strategy ...`、`from quantbase.data ...`、`Strategy`、`DataFeed`、`ctx`、`strategy(ctx)`、`setup(ctx)`。
