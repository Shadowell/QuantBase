...
class FeishuNotifier:
    ...
    def _build_card(title: str, content: str, color: str = "blue") -> dict:
        ...
        elements.append({
            "tag": "note",
            "elements": [
                {"tag": "plain_text", "content": f"QuantBase · {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"}
            ],
        })
    ...
    async def _upload_image_to_feishu(self, image_bytes: bytes) -> str:
        ...
        async with httpx.AsyncClient(timeout=20, trust_env=False) as client:
            ...
            upload_resp = await client.post(
                ...
                files={"image": ("quantbase-strategy-profit.png", image_bytes, "image/png")},
            )
    ...
    def _build_strategy_profit_report_png(cls, report: Dict[str, Any]) -> bytes:
        ...
        draw.text((margin, height - footer_h + 14), "数据来自 QuantBase 当前运行中策略快照", font=fonts["small"], fill="#697386")
    ...
    def _draw_profit_summary_image(cls, draw: Any, report: Dict[str, Any], x: int, y: int, w: int, h: int, fonts: Dict[str, Any]) -> None:
        ...
        items = [
            ...
            ("总盈亏", cls._signed_usd(total_pnl), f"共 {total_trades} 笔交易", "red" if total_pnl > 0 else "green" if total_pnl < 0 else "gray"),
            ("收益率", cls._signed(total_return_pct, suffix="%"), "按初始资金加权", "red" if total_return_pct > 0 else "green" if total_return_pct < 0 else "gray"),
            ("胜率", f"{win_rate:.1f}%", f"{winning_trades}/{closing_trades} 笔平仓盈利" if closing_trades > 0 else "暂无平仓样本", "blue"),
    ...
    def _draw_strategy_profit_image_card(cls, draw: Any, item: Dict[str, Any], x: int, y: int, w: int, h: int, fonts: Dict[str, Any]) -> None:
        ...
        cls._draw_text_ellipsis(draw, f"{trades} 笔交易", (x + w - 32, tag_y + 6), fonts["small"], "#8b95a5", 180, anchor="ra")
    ...
    def _build_strategy_profit_elements(cls, item: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        elements.append(
            {
                "tag": "markdown",
                "content": f"{symbol_badges}　<font color='grey'>{trades} 笔交易</font>",
            }
        )
        return elements
