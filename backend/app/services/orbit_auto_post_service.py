...
def _publisher_subprocess_env() -> Dict[str, str]:
    env = dict(os.environ)
    if env.get("QUANTBASE_ORBIT_USE_SYSTEM_PROXY") == "1":
        return env
...
class BrowserOrbitPublisher:
    ...
    def __init__(self, command: Optional[str] = None, timeout_sec: int = 90) -> None:
        self.command = command or os.getenv("QUANTBASE_ORBIT_POST_COMMAND") or self._default_command()
...
class OrbitAutoPostService:
    ...
    async def preview_candidates(self) -> List[Dict[str, Any]]:
        cfg = self.get_config()
    ...
    async def run_once(self, *, force: bool = False) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            try:
                candidates = await self.preview_candidates()
