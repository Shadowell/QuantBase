...
from app.services.superpnl_feature_builder import canonical_bar_timestamp_ms, normalize_quantbase_symbol
...
class SuperPnL15mLowTurnoverStrategy(BaseStrategy):
    ...
    def _state_for(self, symbol: str) -> SymbolState:
        normalized = normalize_quantbase_symbol(symbol)
    ...
    def _is_trade_symbol(self, symbol: str) -> bool:
        ...
        if not trade_symbols:
            trade_symbols = {normalize_quantbase_symbol(s) for s in self.symbols()}
        return normalize_quantbase_symbol(symbol) in trade_symbols
    ...
    def _resolve_trade_symbols(self) -> set[str]:
        ...
        if raw is None:
            return {normalize_quantbase_symbol(symbol) for symbol in self.symbols() if symbol}
        if isinstance(raw, str):
            ...
            values = []
        symbols = {normalize_quantbase_symbol(symbol) for symbol in values if symbol}
        return symbols or {normalize_quantbase_symbol(symbol) for symbol in self.symbols() if symbol}
    ...
    def _resolve_risk_blacklist(self) -> set[str]:
        ...
        if isinstance(raw, str):
            ...
            values = []
        return {normalize_quantbase_symbol(symbol) for symbol in values if symbol}
    ...
    def _is_symbol_blacklisted(self, symbol: str) -> bool:
        return normalize_quantbase_symbol(symbol) in getattr(self, "_risk_blacklisted_symbols", set())
    ...
    def _resolve_signal_universe_symbols(self) -> set[str]:
        service_symbols = {
            normalize_quantbase_symbol(symbol)
            for symbol in superpnl_model_inference_service.universe_symbols
            if symbol
        }
        if service_symbols:
            return service_symbols
        return {normalize_quantbase_symbol(symbol) for symbol in self._all_known_symbols() if symbol}
    ...
    def _mark_signal_bar_seen(self, timestamp_ms: int, symbol: str) -> tuple[int, int, list[str]]:
        ...
        normalized_symbol = normalize_quantbase_symbol(symbol)
    ...
    def _claim_no_signal_symbol_diag(self, timestamp_ms: int, symbol: str) -> bool:
        key = (canonical_bar_timestamp_ms(int(timestamp_ms)), normalize_quantbase_symbol(symbol))
