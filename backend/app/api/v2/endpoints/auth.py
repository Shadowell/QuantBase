...
def _set_session_cookie(response: Response, session: dict) -> None:
    response.set_cookie(
        settings.QUANTBASE_AUTH_COOKIE_NAME,
        session["token"],
        httponly=True,
        secure=bool(settings.QUANTBASE_AUTH_COOKIE_SECURE),
        samesite="lax",
...
async def me(request: Request):
    token = request.cookies.get(settings.QUANTBASE_AUTH_COOKIE_NAME)
    session = auth_service.get_session(token) if settings.QUANTBASE_AUTH_ENABLED else None
    return ok(_public_session(session, auth_enabled=bool(settings.QUANTBASE_AUTH_ENABLED)))


@router.post("/admin/login")
async def admin_login(payload: AdminLoginRequest, request: Request, response: Response):
    if not settings.QUANTBASE_AUTH_ENABLED:
        return ok(_public_session(None, auth_enabled=False))
    try:
        auth_service.validate_admin_config(
            enabled=True,
            username=settings.QUANTBASE_ADMIN_USERNAME,
            password_hash=settings.QUANTBASE_ADMIN_PASSWORD_HASH,
        )
        session = auth_service.login_admin(
            username=payload.username,
            password=payload.password,
            expected_username=str(settings.QUANTBASE_ADMIN_USERNAME or ""),
            expected_password_hash=str(settings.QUANTBASE_ADMIN_PASSWORD_HASH or ""),
            ip_address=_client_ip(request),
            user_agent=_user_agent(request),
            session_hours=int(settings.QUANTBASE_ADMIN_SESSION_HOURS),
        )
...
async def logout(request: Request, response: Response):
    ...
    response.delete_cookie(settings.QUANTBASE_AUTH_COOKIE_NAME, path="/")
