...
AUTONOMOUS_LLM_PROVIDERS = {"dashscope", "qwen", "hermes", "grok", "xai", "xai-oauth", "xai_oauth"}
...
def _select_llm_model(model: Optional[str] = None) -> str:
    ...
    return str(get_agent_model_config().get("model") or "").strip()
...
def _normalize_autonomous_llm_provider(raw: Any) -> str:
    ...
    return "hermes" if normalized in {"hermes", "grok", "xai", "xai-oauth"} else "dashscope"
...
@router.get("/model-config", summary="读取全局大模型配置（兼容旧 AI Lab 入口）")
async def get_model_config():
    return get_agent_model_config()


@router.put("/model-config", summary="更新全局大模型名称（兼容旧 AI Lab 入口）")
async def update_model_config(req: AgentModelConfigRequest):
    try:
        return await set_agent_model_name(req.model)
    except ValueError as e:
        raise HTTPException(400, str(e))


@router.post("/model-config/test", summary="测试全局大模型连接（兼容旧 AI Lab 入口）")
async def test_model_config():
    ...
    try:
        ...
        logger.warning("AI Lab 模型连接测试失败: %s", describe_qwen_exception(e))
        raise HTTPException(502, f"模型连接测试失败: {describe_qwen_exception(e)}")
...
def _validate_autonomous_request(req: AutonomousTraderStartRequest) -> None:
    ...
    if req.max_total_exposure_pct <= 0 or req.max_total_exposure_pct > 100:
        raise HTTPException(400, "总风险敞口上限必须在 0% 到 100% 之间")
    ...
    if req.probe_size_pct is not None:
        ...
        single_fraction = _as_autonomous_fraction(req.max_single_position_pct, 0.20)
        if probe_fraction <= 0:
...
def _validate_autonomous_config_update(
    ...
    if "max_leverage_cap" in raw_updates:
        ...
        config["default_decision_leverage"] = min(float(config.get("default_decision_leverage") or 2.0), value)
    ...
    single_fraction = _as_autonomous_fraction(config.get("max_single_position_pct"), 0.20)
...
async def start_autonomous_trader(req: AutonomousTraderStartRequest):
    ...
    llm_model = (
        ...
        else str(get_agent_model_config().get("model") or "").strip()
    )
    operator_prompt = str(req.operator_prompt or "").strip()
    ...
    single_position_fraction = _as_autonomous_fraction(req.max_single_position_pct, 0.20)
    probe_size_fraction = (
    ...
    config = {
        ...
        "default_decision_leverage": min(2.0, float(req.max_leverage_cap)),
        "ai_autonomous_trader": True,
    }
    strategy_id = db.save_strategy(
        name=strategy_name,
