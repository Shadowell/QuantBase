...
async def lifespan(app: FastAPI):
    """应用生命周期管理"""
    # 启动时
    logger.info("Starting QuantBase application...")

    # 初始化数据库
    db.init_db()
    logger.info("Database initialized")

    if settings.QUANTBASE_AUTH_ENABLED:
    ...
    if interrupted_agent_count:
        try:
            from app.api.endpoints.agent import auto_resume_interrupted_agent_tasks
    ...
    try:
        from app.api.endpoints.agent import auto_resume_auto_agent_research_runs
    ...
    # 关闭时
    logger.info("Shutting down QuantBase application...")
