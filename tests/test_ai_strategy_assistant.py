...
from app.api.endpoints.agent import (  # noqa: E402
...
def test_ai_strategy_assistant_api_exposes_blueprint_and_local_cycle() -> None:
    ...
    assert payload["execution_plan"]["target"] == "QuantBase paper/simulation only"
...
def test_auto_agent_research_run_persists_for_resume(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.api.endpoints.agent._load_auto_agent_research_runs", lambda: dict(store))

    def fake_save_runs(runs):
        store.clear()
        store.update(runs)

    monkeypatch.setattr("app.api.endpoints.agent._save_auto_agent_research_runs", fake_save_runs)
    monkeypatch.setattr("app.api.endpoints.agent._schedule_auto_agent_research_run", lambda run_id: True)
...
def test_auto_agent_scheduled_scan_uses_builtin_prompt_and_stays_paper_only(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.api.endpoints.agent._save_auto_agent_research_runs", fake_save_runs)
    monkeypatch.setattr("app.api.endpoints.agent._schedule_auto_agent_research_run", lambda run_id: True)
    monkeypatch.setattr("app.api.endpoints.agent._load_auto_agent_scheduler_config", lambda: {
        "enabled": True,
        "interval_minutes": 60,
    ...
    monkeypatch.setattr("app.api.endpoints.agent._save_auto_agent_scheduler_config", lambda cfg: scheduler_cfg.update(cfg) or cfg)
...
def test_auto_agent_scheduler_api_can_enable_fixed_interval_scan(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.api.endpoints.agent._load_auto_agent_scheduler_config", lambda: saved or {
        "enabled": False,
        "interval_minutes": 60,
        "symbols": ["BTC/USDT:USDT"],
        "use_hermes_agent": True,
    ...
    monkeypatch.setattr("app.api.endpoints.agent._save_auto_agent_scheduler_config", lambda cfg: saved.update(cfg) or saved)
