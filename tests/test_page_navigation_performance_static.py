...
def test_market_ai_prediction_stays_off_until_manual_toggle() -> None:
    source = read_text("frontend/src/pages/Market.tsx")

    assert "const [showPrediction, setShowPrediction] = useState(false)" in source
    assert "MARKET_PREDICTION_AUTO_ENABLE_DELAY_MS" not in source
    assert "predictionAutoTimerRef" not in source
    assert "predictionUserTouchedRef" not in source
    assert "setShowPrediction(true)" not in source

    initial_state_pos = source.index("const [showPrediction, setShowPrediction] = useState(false)")
    prediction_call_pos = source.index("const predictionRequest = marketApi.getPredictionsCompare")
    assert initial_state_pos < prediction_call_pos
...
def test_market_current_price_is_rendered_after_chart_selector_controls() -> None:
    ...
    chart_header = source[chart_start:source.index("{showPrediction && (", chart_start)]
    ...
    assert "右侧工具区 — AI 预测 · 刷新 · 连接状态 · 价格" not in toolbar
    assert "右侧工具区 — AI 预测 · 刷新 · 连接状态" in toolbar
...
def test_sidebar_route_icons_match_page_header_icons() -> None:
    ...
    for path, icon, label, page_path, page_icon in page_expectations:
        assert f"{{ path: '{path}', icon: {icon}, label: '{label}' }}" in layout
...
def test_backtest_route_does_not_statically_import_echarts() -> None:
    source = read_text("frontend/src/pages/Backtest.tsx")
...
def test_backtest_trade_statistics_put_profit_factor_after_win_rate() -> None:
    source = read_text("frontend/src/pages/Backtest.tsx")
...
def test_backtest_page_supports_parallel_instances() -> None:
    ...
    assert "const BACKTEST_INSTANCES_KEY = 'quantbase_backtest_instances_v1'" in source
    ...
    assert "回测实例控制台" in source
    assert "创建回测实例" in source
    assert "回测实例" in source
    assert "space-y-3" in source
    assert "md:grid md:grid-cols-[minmax(420px,1.75fr)_minmax(260px,0.75fr)_minmax(300px,auto)]" in source
    assert "xl:grid-cols-[minmax(560px,2fr)_minmax(320px,0.8fr)_minmax(300px,auto)]" in source
    ...
    assert "md:flex-row md:items-center md:justify-end" in source
    assert "BACKTEST_INSTANCE_ACTION_BUTTON_BASE" in source
    assert "backtestInstanceActionButton inline-flex h-10 min-w-[92px]" in source
...
def test_watch_market_page_is_read_only_and_defaults_to_15m_with_trade_markers() -> None:
    ...
    assert "liveExecutionApi.listPositions(ACCOUNT_ID)" in page
    assert "liveExecutionApi.listOrderHistory(ACCOUNT_ID, undefined, 100)" in page
    ...
    assert "watchPositionsColumn flex h-[420px]" in panels
    assert "watchTilesColumn flex h-[420px]" in page
    ...
    assert "watchTilesBody min-h-0 flex-1 overflow-y-auto p-3" in page
    ...
    assert "useTickerWebSocket('okx', symbol)" in page
    ...
    assert "grid grid-cols-1 gap-4 lg:grid-cols-2" in page
    ...
    assert "APP_DEFAULT_VISIBLE_CANDLES = 42" in kline
...
def test_watch_market_reuses_live_account_summary_panels_with_close_actions() -> None:
    ...
    assert "flex h-[420px] min-h-0 min-w-0 flex-col" in panel
    ...
    assert "liveExecutionApi.closePosition(ACCOUNT_ID" in page
...
def test_backtest_result_detail_card_does_not_repeat_strategy_title_and_labels_range() -> None:
    source = read_text("frontend/src/pages/Backtest.tsx")
    result_card = source[
        source.index("{/* ====== BigQuant 风格 - 概要指标行 ====== */"):
        source.index("{/* Tab 切换 */")
    ]

    assert "回测时间范围" in result_card
    assert "result.strategyName || '策略回测结果'" not in result_card
    assert "策略名称" not in result_card
    assert "已落库" not in result_card
...
def test_backtest_trade_table_shows_contract_execution_metrics() -> None:
    ...
    trade_tab_start = source.index("{/* ====== 交易记录 Tab ====== */}")
...
def test_backtest_benchmark_uses_selected_date_range_for_klines() -> None:
    ...
    assert "caption: `vs ${benchmarkSymbol}`" in source
    assert "策略净收益 - ${benchmarkSymbol} 基准收益" in source
...
def test_backtest_overview_metrics_explain_formulae_and_alpha() -> None:
    ...
    assert "function CryptoMetricGroup" in source
    assert "title: '收益表现'" in source
    assert "title: '风险控制'" in source
    assert "title: '交易质量'" in source
    assert "title: '成本与频率'" in source
    assert "label: '净收益'" in source
    assert "label: '基准超额'" in source
    assert "label: '年化波动率'" in source
    assert "label: '手续费占本金'" in source
    assert "label: '赔率'" in source
    assert "label: '期望/笔'" in source
    assert "label: '贝塔'" not in source
    assert "description: '期末权益相对初始资金的净收益率" in source
    assert "description: `策略净收益 - ${benchmarkSymbol} 基准收益" in source
    assert "value: benchmarkStats.alpha != null ? fmtPct(benchmarkStats.alpha) : '-'" in source
    ...
    assert "title={metric.description}" in source
    assert "aria-label={`${metric.label}：${metric.description}`}" in source
    assert "min-h-[88px]" in source
    assert "text-[clamp(0.875rem,0.82vw,1.05rem)] font-semibold leading-snug tracking-normal tabular-nums opacity-90" in source
    ...
    assert "mt-2 flex items-center gap-1.5 text-[11px] font-medium text-gray-300" in source
...
def test_backtest_instance_metric_labels_are_readable() -> None:
    ...
    assert "mt-1 text-[11px] font-medium text-gray-500" in mini_metric
...
def test_backtest_overview_replaces_equity_curve_with_kline_review() -> None:
    ...
    assert "function buildBacktestChartData" in source
    assert "rangeReturn" in source
    assert "rangeMaxDrawdown" in source
    assert "resultTab !== 'overview' && resultTab !== 'trades'" in source
...
def test_nginx_templates_compress_and_cache_static_assets() -> None:
    for path in ("deploy/quantbase.nginx", "deploy/setup-server.sh"):
