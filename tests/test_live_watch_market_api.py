...
def _temp_db(tmp_path, monkeypatch) -> LocalDatabase:
    database = LocalDatabase(str(tmp_path / "quantbase-live-watch.db"))
    database.init_db()
...
def test_live_watchlist_only_returns_quantbase_strategy_live_order_symbols(tmp_path, monkeypatch):
