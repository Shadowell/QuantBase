...
def test_data_sync_client_uses_v2_sync_routes_for_data_page_actions() -> None:
    ...
    assert "postReq('/data_sync/daily_update'" not in data_sync_api
    assert "postReq('/data_sync/delete'" not in data_sync_api


def test_legacy_data_sync_quick_sync_rejects_global_concurrency() -> None:
    source = read_text("backend/app/api/endpoints/data_sync.py")
    quick_sync = source[source.index("def _has_running_quick_sync") : source.index('@router.get("/sync_task/{task_id}")')]
    ...
    assert 'return {"task_id": _sync_tasks[key]["task_id"], "message": "该数据已在同步中", "duplicate": True}' in quick_sync
...
def test_data_manager_exposes_add_symbol_action_in_filter_row() -> None:
    ...
    assert "marketApi.getSymbols(selectedExchange, 'USDT', 'spot')" in source
...
def test_data_manager_symbol_list_includes_synced_and_running_job_symbols() -> None:
    ...
    assert "setScheduleSymbols(scheduleSymbols.length === configuredSymbols.length ? [] : [...configuredSymbols])" in schedule_dialog
    assert "configuredSymbols.map((sym)" in schedule_dialog
...
def test_data_manager_add_symbol_can_submit_spot_and_swap_together() -> None:
    source = read_text("frontend/src/pages/DataManager.tsx")
...
def test_data_manager_add_symbol_can_start_history_sync_for_new_pairs() -> None:
    ...
    assert "近1年" in source
    assert "近半年" in source
    assert "近3月" in source
    assert "近1月" in source
    assert "useState(365)" in source
    ...
    assert "timeframes: allTimeframes" in source
...
def test_data_manager_detail_sections_do_not_flex_shrink() -> None:
    ...
    assert 'className="grid grid-cols-5 gap-3 shrink-0"' in source
...
def test_data_manager_supports_30m_timeframe_when_synced_metadata_contains_it() -> None:
    ...
    assert "const TIMEFRAME_ORDER = ['1m', '5m', '15m', '30m', '1h', '4h', '1d']" in source
