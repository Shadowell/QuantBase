...
def build_client(tmp_path: Path, monkeypatch) -> tuple[TestClient, AuthService]:
    ...
    monkeypatch.setattr(settings, "QUANTBASE_AUTH_ENABLED", True, raising=False)
    monkeypatch.setattr(settings, "QUANTBASE_ADMIN_USERNAME", "admin", raising=False)
    monkeypatch.setattr(settings, "QUANTBASE_ADMIN_PASSWORD_HASH", password_hash, raising=False)
    monkeypatch.setattr(settings, "QUANTBASE_AUTH_COOKIE_SECURE", False, raising=False)
    ...
    @app.get("/api/v2/trading/balance")
...
def test_admin_login_sets_cookie_and_can_call_protected_write(tmp_path: Path, monkeypatch) -> None:
    ...
    assert int(cookie[settings.QUANTBASE_AUTH_COOKIE_NAME]["max-age"]) > 60 * 60 * 24 * 365
