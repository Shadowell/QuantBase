...
def test_websocket_endpoint_forwards_timeframe_to_backend_subscriptions():
    source = (ROOT / "backend/app/api/endpoints/websocket.py").read_text()

    assert 'timeframe = message.get("timeframe")' in source
    assert "connection_manager.subscribe(\n                        websocket, channel, exchange, symbol, timeframe" in source
    assert "connection_manager.unsubscribe(\n                        websocket, channel, exchange, symbol, timeframe" in source
