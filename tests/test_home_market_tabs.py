...
def test_market_universe_overview_renders_okx_app_style_rankings():
    ...
    assert "HOME_SUMMARY_SYMBOLS" in text
    assert "variant === 'summary'" in text
    assert "items={homeRankings[homeRankingKey]}" in text
    assert "grid-cols-[minmax(160px,1fr)_116px_92px_128px_132px_132px_minmax(260px,1fr)]" in text
...
def test_home_summary_fuses_market_overview_and_sentiment_index():
    ...
    assert "QuantBase 大盘情绪指数" not in text
