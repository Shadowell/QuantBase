...
def test_market_ai_prediction_fetches_indicators_for_actual_compare_kline_window() -> None:
    market_source = open("frontend/src/pages/Market.tsx", encoding="utf-8").read()

    assert "const compareKlines = (cmp.klines || []).slice(-klineLimit)" in market_source
    assert "const indicatorStart = compareKlines[0]?.timestamp ?? startTime" in market_source
    assert "const indicatorEnd = compareKlines[compareKlines.length - 1]?.timestamp ?? endTime" in market_source
    assert "indicatorStart," in market_source
    assert "indicatorEnd," in market_source
    assert "setKlines(compareKlines)" in market_source
