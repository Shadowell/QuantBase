...
def test_monitor_top_summary_includes_live_account_cards():
    ...
    assert 'label="实盘策略"' in live_section
    ...
    assert "value={String(liveMonitorSummary.deployedCount)}" in live_section
...
def test_monitor_live_summary_derives_today_and_risk_metrics():
    ...
    assert "const todayRealizedPnl = todayOrders.reduce((sum, order) => sum + finiteNumber(order.pnl), 0);" in text
    assert "const todayFees = todayOrders.reduce((sum, order) => sum + Math.abs(finiteNumber(order.fee)), 0);" in text
...
def test_monitor_running_strategy_cards_use_dollar_amounts():
    ...
    assert "{formatSignedUsd(s.pnl)}" in text
...
def test_monitor_page_places_config_below_metrics_and_shows_live_monitor_beside_paper():
    ...
    assert "const fetchLiveMonitor = async () =>" in text
