...
def test_live_execution_frontend_api_uses_live_strategy_endpoints():
    ...
    assert "pnl?: number | null" in client
    assert "quantbaseSource?: 'strategy' | 'external'" in client
    assert "quantbaseSourceLabel?: string | null" in client
...
def test_live_strategy_list_does_not_expose_remove_action():
    page = _read("frontend/src/pages/liveTrading/LiveExecutionCenter.tsx")

    assert "removeStrategy" not in page
    assert "移出实盘策略列表" not in page
    assert "liveExecutionApi.updateStrategy(strategyId, { added: false })" not in page
...
def test_live_account_management_actions_are_inline_and_filled():
    ...
    assert 'wrapperClassName="min-w-[280px] flex-1 sm:w-[360px] sm:flex-none"' in account_header
...
def test_live_execution_center_exposes_signal_style_workbench():
    ...
    assert "选择账户 / 新增 OKX API Key" in page
    assert "增加账户" in page
    ...
    assert "+ 绑定账户" in page
    assert "border-crypto-border bg-crypto-bg text-gray-100" in page
    assert "border-crypto-border bg-crypto-bg text-gray-200 hover:border-blue-500/35 hover:text-blue-200 hover:bg-white/[0.03]" in page
    ...
    assert "{bound ? '已绑定' : '未绑定'}" in page
    assert "bound ? 'bg-green-500/15 text-green-300' : 'bg-gray-500/15 text-gray-400'" in page
    ...
    assert "border-crypto-border bg-crypto-bg px-3 text-xs font-semibold text-gray-100 hover:border-blue-500/35" in page
    ...
    assert "实盘预检请求失败" in page
    assert "failedPreflight(" in page
    assert "账户绑定检查" in page
    assert "实盘预检" in page
    assert "预检结果" in page
    assert "实盘部署流水线" in page
    assert 'aria-label="实盘部署流水线"' in page
    assert page.count("实盘部署流水线") == 2
    assert page.count("renderDeployPipeline()") == 1
    assert page.index("使用的实盘账户") < page.index("renderDeployPipeline()") < page.index("实盘面板")
    assert "renderDeployPipeline()" not in page[page.index("实盘面板") :]
    assert "deploymentPipelineStepBase" in page
    assert "rounded-xl border border-crypto-border bg-crypto-bg p-1" in page
    assert "grid-cols-[minmax(0,1fr)_1px_minmax(0,1fr)_1px_minmax(0,1fr)]" in page
    assert "h-7 w-px bg-crypto-border/70" in page
    assert "left-[16.66%] right-[16.66%]" not in page
    assert "top-[1.375rem]" not in page
    ...
    assert "确认部署实盘" in page
    assert "ThemeDialog" in page
    assert "confirmPaperReviewed: true" in page
    assert "confirmLiveRisk: true" in page
    assert "accountId: selectedAccountId" in page
    assert "将为当前模拟策略创建" in page
    assert "实盘执行订阅" in page
    assert "将克隆独立实盘策略" not in page
    assert "preflightKey(strategy.strategyId, selectedAccountId)" in page
    assert "deployReady" in page
    assert "deploymentPipelineDisabled" in page
    assert "deploymentPipelineStepBase" in page
    assert "liveActionButtonWarning" in page
    assert "liveActionButtonDanger" in page
    assert "const visiblePreflight = deploymentPipelineDisabled ? undefined : activePreflight" in page
    assert "disabled={deploymentPipelineDisabled || !selectedAccountBound || selectedStrategyBusy}" in page
    assert "disabled={deploymentPipelineDisabled || !activePreflight}" in page
    assert "loadStrategies({ silent: true })" in page
    assert "window.setInterval" in page
    assert "window.clearInterval(timer)" in page
    assert "await loadAccountSnapshot()" in page
    assert "实盘操作" not in page
    assert "disabled={deploymentPipelineDisabled || !deployReady || selectedStrategyBusy}" in page
    assert "openDeployConfirm(selectedStrategy)" in page
    assert "openDeployConfirm(strategy)" not in page
    ...
    assert "grid grid-cols-1 gap-3 xl:grid-cols-3" in page
    ...
    assert "asset_type" in _read("backend/app/api/v2/endpoints/live.py")
    ...
    assert "QuantBase 策略信号" in summary_panels
    ...
    assert "void runPreflight(selectedStrategy)" in page
