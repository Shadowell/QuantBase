...
def test_optimizer_prompt_forbids_legacy_quantbase_strategy_imports():
    ...
    assert "禁止 `from quantbase.strategy" in prompt
    assert "from app.core.execution.base_strategy import BaseStrategy, BarData" in prompt
    assert "strategy(ctx)" in prompt


def test_generate_candidate_retries_when_ai_uses_legacy_quantbase_imports(monkeypatch):
    now = datetime(2026, 5, 4, 8, 0, tzinfo=timezone.utc)
    db = FakeDb(now)
    svc = StrategyOptimizerService(database=db, engine=FakeEngine(), now_fn=lambda: now)
        ...
        async def chat_json(self, messages, **kwargs):
            ...
            if len(self.calls) == 1:
                return {
                    ...
                    "strategy_class_code": (
                        "from quantbase.strategy import Strategy\n"
                        "from quantbase.data import Bar\n\n"
    ...
    assert "quantbase.strategy" in retry_feedback
