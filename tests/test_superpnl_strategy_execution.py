...
def test_high_win_profit_protected_superpnl_seed_clones_aggressive_top5():
    seed_path = ROOT / "data" / "seed" / "strategies.json"
    strategies = json.loads(seed_path.read_text())
    source = next(item for item in strategies if item["name"] == "[现货] SuperPnL 高流动性小币 Top5 · 激进单仓")
    clone = next(
        item
        for item in strategies
        if item["name"] == "[现货] SuperPnL 高流动性小币 Top5 · 高胜率浮盈保护"
    )

    assert clone["strategy_key"] == source["strategy_key"]
    assert clone["symbols"] == source["symbols"]
    assert clone["config"]["trade_symbols"] == source["config"]["trade_symbols"]
    assert clone["config"]["threshold_bps"] > source["config"]["threshold_bps"]
    assert clone["config"]["threshold_bps"] == 40
    assert clone["config"]["top_k"] == 2
    assert clone["config"]["max_total_position"] == 0.3
    assert clone["config"]["take_profit_bps"] > 0
    assert clone["config"]["trailing_start_bps"] > clone["config"]["profit_floor_bps"]
    assert clone["config"]["rolling_win_rate_min_trades"] >= 4
    assert clone["config"]["rolling_win_rate_threshold"] >= 0.55
    assert clone["config"]["max_symbol_consecutive_losses"] >= 2
    assert clone["config"]["symbol_loss_cooldown_bars"] >= 120


def test_mainstream_liquidity_superpnl_seed_uses_major_trade_subset():
    seed_path = ROOT / "data" / "seed" / "strategies.json"
    strategies = json.loads(seed_path.read_text())
    source = next(item for item in strategies if item["name"] == "[现货] SuperPnL 高流动性小币 Top5 · 激进单仓")
    major = next(
        item
        for item in strategies
        if item["name"] == "[现货] SuperPnL 高流动性主流币 Top3 · 高胜率浮盈保护"
    )

    assert major["strategy_key"] == source["strategy_key"]
    assert major["symbols"] == source["symbols"]
    assert major["config"]["trade_symbols"] == ["BTC/USDT", "ETH/USDT", "SOL/USDT"]
    assert major["config"]["threshold_bps"] >= 55
    assert major["config"]["top_k"] == 1
    assert major["config"]["max_total_position"] <= 0.25
    assert major["config"]["min_bar_quote_volume_usdt"] >= 50_000
    assert major["config"]["take_profit_bps"] > 0
    assert major["config"]["trailing_start_bps"] > major["config"]["profit_floor_bps"]
    assert major["config"]["rolling_win_rate_threshold"] >= 0.55
    assert major["config"]["max_symbol_consecutive_losses"] >= 2
...
def test_contract_mainstream_superpnl_seed_is_paper_swap_with_spot_signal_feed():
    seed_path = ROOT / "data" / "seed" / "strategies.json"
    strategies = json.loads(seed_path.read_text())
    contract = next(
        item
        for item in strategies
        if item["name"] == "[合约] SuperPnL 高流动性主流合约 Top3 · 高胜率浮盈保护"
    )

    cfg = contract["config"]
    assert contract["strategy_key"] == "superpnl_contract_mainstream"
    assert cfg["market_type"] == "swap"
    assert cfg["is_paper_trading"] is True
    assert cfg["signal_feed_market_type"] == "spot"
    assert cfg["trade_symbols"] == ["BTC/USDT:USDT", "ETH/USDT:USDT", "SOL/USDT:USDT"]
    assert all(not symbol.endswith(":USDT") for symbol in contract["symbols"])
    assert cfg["leverage"] <= cfg["max_leverage"]
