...
def test_strategies_without_page_keeps_legacy_array_response(monkeypatch) -> None:
    async def fake_list():
        return [{"id": 1, "name": "legacy"}]

    monkeypatch.setattr(strategy.strategy_domain_service, "list", fake_list)
    ...
    assert body["data"] == [{"id": 1, "name": "legacy"}]
...
def test_strategies_with_page_returns_paginated_payload(monkeypatch) -> None:
    ...
    async def fake_list_page(**kwargs):
        ...
        return {
            ...
            "capital_counts": {"all": 4, "100U": 3, "1000U": 1, "10000U": 0},
        }
...
def test_strategy_page_service_filters_and_counts_by_existing_asset_contracts(monkeypatch) -> None:
    ...
    assert result["capital_counts"] == {"all": 1, "100U": 1, "1000U": 0, "10000U": 0}
...
def test_strategy_search_matches_like_simulation_without_rewriting_counts(monkeypatch) -> None:
    ...
    assert result["timeframe_counts"]["15m"] == 1
    assert result["capital_counts"] == {"all": 4, "100U": 4, "1000U": 0, "10000U": 0}
